...
class UserTableViewCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        ...
        self.unmatchButton.addTarget(self, action: #selector(unmatchTapped(_:)), for: .touchUpInside)
        
    }
    
    
    
    @IBAction func unmatchTapped(_ sender: Any) {
        if let user = user,
            let delegate = delegate {
            self.delegate?.MatchesTableViewCell(self, subscribeButtonTappedFor: user)
        }
